...
namespace solver {
	...
	void insert(string s, int w) {
		...
		rep(i, 0, n - 1) {
			...
			if(i == n - 1) v[cur].dp[val] += w, print(cur);
			cur = v[cur].to[val];
		}
		return;
	}
	...
	void build() {
		...
		dfs(0, {0, 0, 0});
		rep(i, 0, ii) rep(j, 0, 2) {
			int val = i;
			while(val && !v[val].to[j]) val = v[val].p;
			if(v[val].to[j]) val = v[val].to[j];
			v[i].rec[j] = val;
		}
	}
	void solve(string s) {
		...
		rrep(i, 0, n - 1) rep(j, 0, ii) {
			int val = id[s[i]];
			if(s[i] == 'x') {
				rep(k, 0, 2) a[i][j] = max(a[i][j], 
					a[i + 1][v[j].rec[k]] + v[j].dp[k]);
			}
			else a[i][j] = max(a[i][j], a[i + 1][v[j].rec[val]] + v[j].dp[val]);
		}
		cout << a[0][0] << "\n";
	}
};
